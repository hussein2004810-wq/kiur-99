...
console.log(`Foreign key integrity check errors: ${fkErrors.length}`);

// Test insert & cascade
db.exec(`
  INSERT INTO sections (id, name) VALUES ('sec_1', 'طب بشري');
  INSERT INTO universities (id, name, section_id) VALUES ('uni_1', 'جامعة بغداد', 'sec_1');
  INSERT INTO stages (id, name, university_id) VALUES ('stg_1', 'المرحلة الأولى', 'uni_1');
  INSERT INTO subjects (id, name, stage_id) VALUES ('sub_1', 'التشريح', 'stg_1');
  INSERT INTO users (id, email, full_name, role) VALUES ('usr_1', 'student@test.com', 'طالب تجريبي', 'student');
  INSERT INTO user_sessions (id, user_id, device_label) VALUES ('ses_1', 'usr_1', 'متصفح كروم');
`);

const sessionBefore = db.prepare("SELECT * FROM user_sessions WHERE id='ses_1'").all();
console.log('Session created:', sessionBefore.length === 1);

// Test cascade delete on user
db.exec("DELETE FROM users WHERE id='usr_1';");
const sessionAfter = db.prepare("SELECT * FROM user_sessions WHERE id='ses_1'").all();
console.log('Session cascade-deleted after user deletion:', sessionAfter.length === 0);

db.close();
console.log('ALL DDL TESTS PASSED PERFECTLY!');
