...
function adminUserOut(user: typeof schema.users.$inferSelect) {
  return {
    id: user.id,
    email: user.email,
...
adminRouter.put('/users/:user_id', async (c) => {
  ...
  const updated = await db.select().from(schema.users).where(eq(schema.users.id, userId)).get();
  return c.json(updated ? adminUserOut(updated) : null);
});
