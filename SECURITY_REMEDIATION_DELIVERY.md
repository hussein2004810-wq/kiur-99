...
- Google Identity Services credentials are now verified with Google's OIDC JWKS using RS256, issuer, audience, issued-at, and expiry checks; the direct credential route no longer treats the `tokeninfo` response as its authentication boundary.
